...
function getTeams(eventCode) {
	const teamsSpan = document.getElementById("span_teams");
	...
	if (authKey) {
		...
		xmlhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				...
				teamsSpan.innerHTML = teams.length + " teams found";
			}
		};
		// Send request
		xmlhttp.send();
...
function getSchedule(eventCode) {
	const scheduleSpan = document.getElementById("span_schedule");
	...
	if (authKey) {
		...
		xmlhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				...
				scheduleSpan.innerHTML = schedule.length + " matches found";
			}
		};
		// Send request
		xmlhttp.send();
